...
and presume that you have cloned a copy of FreeCAD's source code, and therefore have the directory _package/WindowsInstaller_.
...
Next, update the installer settings for the current version of FreeCAD. Starting from the _package/WindowsInstaller_ folder in the FreeCAD source tree:
...
2. If the installer will be made from a LibPack build create a new folder called MSVCRedist within the folder _package/WindowsInstaller_ and copy the following files from your MSVC installation into it:
...
4. Ensure the FreeCAD files are in place. Here you have two options:
   ...
   - If you compiled FreeCAD on your own as described [here](https://wiki.freecad.org/Compile_on_Windows) (and using the Install option outlined there). Then:
     - Copy into the installation folder the file _Delete.bat_ that is part of the installer
...
5. Right-click on the file _FreeCAD-installer.nsi_ and choose **Compile NSIS script**
...
in the file _Settings.nsh_ or by defining `FC_TEST_BUILD` in command line
...
Interactive installers check Dropsite before installing an older bundled build. A production installer must be Authenticode-signed; unsigned local builds skip automatic updates.

Dropsite only offers an exact Parashell Windows x86-64 installer with a positive byte size and matching SHA-256 sidecar. When accepted, the signed bootstrap installs its updater controller in an administrator-protected directory under `%ProgramData%\Parashell\Updater`, then closes while Windows BITS transfers the update in the background.

The controller uses a global mutex and deterministic BITS job name so concurrent launches reuse one transfer. Downloads are resumable, use BITS retry and no-progress timeouts, and are staged under `%ProgramData%\Parashell\Updates`. Before launch, the controller verifies the expected size, SHA-256, Authenticode chain, and signer subject inherited from the trusted bootstrap. It atomically promotes the verified partial file, waits for the old bootstrap to exit, launches the new installer, records status in `state.json`, and removes completed artifacts. Failed verification never launches the file and displays an update error.

Publishing a signed installer requires Azure Artifact Signing. When signing is available, the bundle script signs the installer, verifies it with `signtool`, then writes the checksum sidecar. When signing is unavailable, the script still builds and uploads an unsigned installer that skips automatic updates, so releases are never blocked on signing access.
