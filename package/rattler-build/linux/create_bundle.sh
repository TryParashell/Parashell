...
cp ${conda_env}/bin_tmp/rg ${conda_env}/bin/
...
version_name="Parashell_${BUILD_TAG}-Linux-$(uname -m)"
...
./appimagetool-$(uname -m).AppImage \
  ...
  --mksquashfs-opt 22 \
  -u "gh-releases-zsync|TryParashell|Parashell|${GH_UPDATE_TAG}|Parashell*$(uname -m)*.AppImage.zsync" \
...
if [ "${UPLOAD_RELEASE}" == "true" ]; then
    ...
    if [ "${GH_UPDATE_TAG}" == "weeklies" ]; then
        generic_name="Parashell_weekly-Linux-$(uname -m)"
